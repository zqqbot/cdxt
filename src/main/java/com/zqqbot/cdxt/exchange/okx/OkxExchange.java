...
import com.zqqbot.cdxt.http.OkxHttpClient;
...
public class OkxExchange extends BaseExchange {
    ...
    private OkxExchangeApi api;
    private OkxExchangeParser parser;
    
    public OkxExchange() {
        this.name = "OKX";
        this.api = new OkxExchangeApi(sandbox, marketType);
        this.parser = new OkxExchangeParser(marketType);
    }
    
    @Override
    protected void initHttpClient() {
        if (apiKey != null && secret != null) {
            this.httpClient = new OkxHttpClient(apiKey, secret, passphrase, sandbox);
        }
    ...
    protected String buildFuturesMarginTypeParams(String symbol, String marginType) {
        return "okx下单时直接通过tdMode参数即可控制全仓或逐仓";
    }
    @Override
